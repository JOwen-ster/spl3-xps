...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-8 gap-12">
      <div className="w-full max-w-2xl sticky top-8 z-10">
        <input
          type="text"
          placeholder="Search Player, XP, Country..."
          className="w-full bg-search-bg text-black/60 text-lg px-8 py-4 rounded-full shadow-lg outline-none placeholder:text-black/40"
          value={searchQuery}
          onChange={handleSearchChange}
        />
      </div>

      <div className="w-full max-w-4xl bg-white rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col">
          ...
          {currentPlayers.map((player) => {
            ...
            return (
              <div
                key={player.name}
                className={cn(
                  "flex items-center justify-between px-12 py-5 transition-colors",
              ...
              >
                <div className="flex-1 flex items-center gap-4">
                  <span
                    className={cn(
                      "flex items-center justify-center w-12 h-12 rounded-full font-bold text-sm shrink-0 border-2 border-current/10",
                ...
                <div className="flex-1 text-center font-bold font-mono text-lg">{player.xp}</div>
                <div className="flex-1 text-right italic opacity-80">{player.country ?? "--"}</div>
              </div>
            );
          })}
